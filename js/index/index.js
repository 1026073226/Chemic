	...
	methods: {
	  specialEqui( effect ) {
					...
					setTimeout(  () => {
						...
						this.inter.load = setTimeout( () => {
							...
							setTimeout( () => {
								...
								this.d.$( "#getIn" ).style.animation =
									"blink 1891ms infinite";
								this.d.$( "#loadPic" ).addEventListener(
									"click", this
									.getIn, {
										once: true
									} );
		...
		initWorker() {
			...
			this.mkMatcher = new Worker( "js/index/match.js" );
			this.matcher.onmessage = ( event ) => {
				this.matched = event.data;
			};
			this.mkMatcher.onmessage = ( event ) => {
				this.proMk = event.data;
			};
			this.workers = [];
		...
		make() {
			this.playSound( "ding" );
			this.maked.push( this.d.$( "#showEle" ).innerHTML );
			this.proMk = [];
			this.a.play( this.d.$( "#showEle" ), "jslidein 286ms" );
		...
		splitCamelCase( str ) {
			...
			while ( ( match = regex.exec( str ) ) !== null ) {
				if ( match[ 1 ] ) {
					...
					// 递归处理括号内的化学式
					const subResult = this.splitCamelCase( subFormula );
		  ...
		  let lastone = keys[ keys.length - 1 ];
			if ( v in this.chemist.elements ) {
			  if ( lastone ) {
				...
				this.text[ lastone ] += v;
				for ( let i = 0; i < v; i++ ) {
					let eles = this.splitCamelCase( lastone );
		...
		checkQueue( array, targetString ) {
			...
			const lastThreeElements = array.slice( -2 );

			return lastThreeElements.every( element => element === targetString );
		},
		tip( msg, rep ) {
			if ( rep && this.inter.tip ) {
				clearTimeout( this.inter.tip );
			}
			if ( !this.checkQueue( this.tipQueue, msg ) || rep ) {
				// 将消息添加到队列
      ...
      `;
		...
		quest() {
			if ( this.quests.length >= 3 || this.questsCounter < 3 ) {
				this.questsCounter++;
				return;
			}
			this.questsCounter = 0;
			this.newPresent();
			let r = this.chemist.x[
				this.randint( 0, this.chemist.x.length - 1 )
			];
      ...
      `,
		...
		initTrollDrag() {
			...
			const onMove = ( clientY ) => {
				if ( this.isDragging === "end" ) return;
				const deltaY = startY - clientY;
				if ( Math.abs( deltaY ) > 15 && this.isDragging === "start" ) {
	...
	watch: {
		...
		"settings.am"() {
			if ( this.settings.am ) {
				...
				this.d.$( "#loadPic" ).style.backgroundColor = "#555";
				document.body.style.color = "#FFF";
				document.body.style.filter = "brightness(0.7) contrast(1.4) grayscale(0.4)";
			} else {
				this.d.$( "#loadPic" ).style.backgroundColor = "#FFF";
				document.body.style.backgroundColor = "#FFF";
