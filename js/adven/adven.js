...
class MainGameScene extends Phaser.Scene {
	...
	create() {
		...
		if (this.isMobile) {
			this.MAX_PARTICLES *= 0.5; // 减少移动端粒子数量
		} else {
			this.changeControls();
		}
	...
	changeControls() {
		if (this.controlsWay == "touch") {
			//关闭触摸控制器
			document.getElementById("controls").querySelectorAll("*:not(#current-effects)").forEach((e) => {
...
class GameOverScene extends Phaser.Scene {
	...
	create() {
		...
		const originalWidth = buttonTexture.getSourceImage().width;
