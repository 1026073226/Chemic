...
class MainGameScene extends Phaser.Scene {
	constructor() {
		super( 'MainGameScene' );
		this.MOVE_SPEED = 600 * GAME_FACTOR;
		this.JUMP_FORCE = -1100 * GAME_FACTOR;
		this.currentEntropy = 0;
		this.bgmIndex = 0;
		this.CHUNK_WIDTH = GAME_WIDTH;
		this.chunkIndex = 0;
		// 新增技能冷却系统
	...
	createTerrainPlatforms() {
		...
		platforms.forEach( config => {
			const platform = this.groundGroup.create(
				...
				.setScale( config.width, 0.5 * GAME_FACTOR )
				.refreshBody();
	...
	spawnEnemy( x, y ) {
		...
		enemy.speed = Phaser.Math.Between( 200, 500 ) * GAME_FACTOR;
		enemy.spawnX = x;
		enemy.moveDirection = 1;
		// 敌人AI行为
		...
		for ( let i = 0; i < times; i++ ) {
			if ( Math.random() > 0.6 ) {
				const deco = this.decorations.create(
					...
					.setScale( Phaser.Math.Between( 0.5, 1 ) * GAME_FACTOR );
    ...
    this.sumFPS ++;
		...
		if ( this.spellCooldown > 0 ) {
			...
			this.cooldownBar.clear()
				...
				.fillRect( 80 * GAME_FACTOR, 150 * GAME_FACTOR, 200 * GAME_FACTOR * ( 1 - this.spellCooldown /
					this
					.MAX_COOLDOWN ), 20 );
		}
	}
	...
	setupJoystick() {
		const x = this.scale.width - 200 * GAME_FACTOR;
		const y = this.scale.height - 200 * GAME_FACTOR;
		...
		this.joystick = this.plugins.get( 'rexVirtualJoystick' ).add( this, {
			x: x, // 摇杆的 x 坐标
			y: y, // 摇杆的 y 坐标
			radius: 70 * GAME_FACTOR, // 摇杆的半径
			forceMin: 0.1, //最小力度
			base: this.add.circle( 0, 0, 65 * GAME_FACTOR, 0xEDEDED ).setAlpha( 0.6 ), // 摇杆底座（可选）
			thumb: this.add.circle( 0, 0, 30 * GAME_FACTOR, this.SPELL_COLOR ).setAlpha(
				0.8 ), // 摇杆拇指区域（可选）
			dir: "8dir",
			fixed: true, // 是否固定位置
			enable: true, // 是否启用
		...
		for ( let i = 0; i < config.attack.count; i++ ) {
			...
			if ( particle ) {
				...
				particle.fire(
					...
					config.attack.speed * GAME_FACTOR,
					1500 * GAME_FACTOR,
					-500 * GAME_FACTOR,
					true,
					"enemy"
...
class HitParticle extends Phaser.Physics.Arcade.Sprite {
	...
	fire( x, y, angle, speed, lifespan ) {
		...
		this.setScale( 0.5 * GAME_FACTOR ); // 缩小粒子尺寸
	}
	preUpdate( time, delta ) {
		super.preUpdate( time, delta );
		this.lifespan -= delta;
