...
var MAX_PARTICLES = QUALITY <= 0 ? 30 : 100 * Math.sqrt(QUALITY);
...
class MainGameScene extends Phaser.Scene {
	...
	turnEnemy(enemy) {
		...
		this.time.delayedCall(100,
			() => {
				if (enemy.active && !enemy.destroyed && enemy.body) {
					// 防止敌人被销毁后操作
	...
	spawnDecorations(platform) {
		...
		for (let i = 0; i < times; i++) {
			if (Math.random() > 0.6) {
				...
				if (QUALITY > 0) deco.preFX.addGlow(0xF9E0A0, 0.5, 0.2, false, 0.025 * QUALITY);
